...
class GlobalCoordinator:
    ...
    def update_global_state(self):
        """更新全局状态信息"""
        # 统计各机场的占位任务紧急度
        self.airport_gd_urgency = {}
        
        # 直接从unassigned_ground_duty_ids获取未分配的占位任务
        for gd_id in self.env.unassigned_ground_duty_ids:
            # 从ground_duties数据中查找对应的占位任务
            gd_row = self.env.dh.data['ground_duties'][self.env.dh.data['ground_duties']['id'] == gd_id]
            if not gd_row.empty:
                gd = gd_row.iloc[0].to_dict()
                airport = gd['airport']
                urgency = self._calculate_gd_urgency(gd)
                
                if airport not in self.airport_gd_urgency:
                    self.airport_gd_urgency[airport] = []
                
                self.airport_gd_urgency[airport].append({
                    'gd_id': gd['id'],
                    'crew_id': gd['crewId'],
                    'urgency': urgency,
                    'start_time': gd['startTime']
                })
        
        # 识别高优先级机场
        self.priority_airports = []
        for airport, gd_list in self.airport_gd_urgency.items():
            total_urgency = sum(gd['urgency'] for gd in gd_list)
            self.priority_airports.append({
                'airport': airport,
                'total_urgency': total_urgency,
                'gd_count': len(gd_list)
            })
        
        self.priority_airports.sort(key=lambda x: x['total_urgency'], reverse=True)
    
    def get_airport_priority_bonus(self, airport):
        """获取机场的优先级奖励"""
        for i, priority_airport in enumerate(self.priority_airports):
            if priority_airport['airport'] == airport:
                # 排名越靠前，奖励越高
                rank_bonus = (len(self.priority_airports) - i) / len(self.priority_airports)
                return rank_bonus * 50.0  # 最高50分奖励
        return 0.0
    
    def suggest_crew_movement(self, crew_info, crew_state):
        """建议机组移动方向"""
        ...
        crew_gds = []
        for gd_id in self.env.unassigned_ground_duty_ids:
            gd_row = self.env.dh.data['ground_duties'][
                (self.env.dh.data['ground_duties']['id'] == gd_id) & 
                (self.env.dh.data['ground_duties']['crewId'] == crew_id)
            ]
            if not gd_row.empty:
                crew_gds.append(gd_row.iloc[0].to_dict())
        
        if not crew_gds:
            return None
        
        # 按紧急度排序
        crew_gds.sort(key=lambda x: self._calculate_gd_urgency(x), reverse=True)
        
        # 返回最紧急的占位任务机场
        most_urgent_gd = crew_gds[0]
        target_airport = most_urgent_gd['airport']
        
        if target_airport != current_location:
            return {
                'target_airport': target_airport,
                'reason': 'urgent_ground_duty',
                'urgency': self._calculate_gd_urgency(most_urgent_gd)
            }
        
        return None
    
    def _calculate_gd_urgency(self, gd):
        """计算占位任务紧急度"""
        current_time = self.env.planning_start_dt + pd.Timedelta(hours=self.env.total_steps * 0.1)
        time_to_start = (gd['startTime'] - current_time).total_seconds() / 3600
        
        if time_to_start < 0:
            return 0.0  # 已过期
        elif time_to_start < 12:
            return 1.0
        elif time_to_start < 24:
            return 0.8
        elif time_to_start < 48:
            return 0.5
        else:
            return 0.3
...
class CrewRosteringEnv:
    ...
    def _calculate_crew_capability_bonus(self, crew_info):
        ...
        try:
            ...
            if base_airport in hub_airports:  # 主枢纽
                return 0.3
            elif base_airport in major_airports:  # 主要枢纽
                return 0.25
    ...
    def _find_multi_step_paths_to_ground_duties(self, crew_info, crew_state, max_depth=2):
        """
        查找多步路径到达占位任务
        返回: {目标占位任务: [可能的第一步动作列表]}
        """
        crew_id = crew_info['crewId']
        current_location = crew_state['last_location']
        current_time = crew_state['last_task_end_time']
        ...
        # 获取未完成的占位任务
        unfinished_gds = [
            gd for gd in self.dh.crew_ground_duties.get(crew_id, [])
            if gd['id'] in self.unassigned_ground_duty_ids
            and gd['startTime'] > current_time
        ]
        
        if not unfinished_gds:
            return {}
        
        # 对每个占位任务，搜索可达路径
        gd_first_steps = {}
        
        for gd in unfinished_gds:
            target_airport = gd['airport']
            target_time = gd['startTime']
            
            # BFS搜索到达目标的路径
            first_steps = self._bfs_search_paths(
                start_location=current_location,
                start_time=current_time,
                target_location=target_airport,
            ...
            if first_steps:
                gd_first_steps[gd['id']] = first_steps
        
        return gd_first_steps
    
    def _bfs_search_paths(self, start_location, start_time, target_location, 
                          target_time, crew_info, max_depth=2):
        """
        使用BFS搜索从起点到目标的所有可能第一步
        """
        from collections import deque
        
        # 队列元素: (当前位置, 当前时间, 深度, 第一步任务)
        queue = deque([(start_location, start_time, 0, None)])
        visited = set()
        first_steps = set()
        
        while queue:
            curr_loc, curr_time, depth, first_step = queue.popleft()
            
            # 达到目标
            if curr_loc == target_location and curr_time <= target_time:
                if first_step:
                    first_steps.add(first_step['taskId'])
                continue
            ...
            # 避免重复访问
            state_key = (curr_loc, curr_time.hour, depth)
            if state_key in visited:
                continue
            visited.add(state_key)
            
            # 探索所有可能的下一步
            next_tasks = self.unified_tasks_df[
                (self.unified_tasks_df['depaAirport'] == curr_loc) &
                (self.unified_tasks_df['startTime'] > curr_time) &
                (self.unified_tasks_df['startTime'] <= target_time)
            ]
            
            for _, task in next_tasks.iterrows():
                # 检查任务是否可执行
                if not self._can_execute_task_type(task, crew_info):
                    continue
                
                # 记录第一步
                next_first_step = first_step if first_step else task.to_dict()
        ...
        return list(first_steps)
    
    def _can_execute_task_type(self, task, crew_info):
        """检查机组是否能执行该类型任务"""
        if task['type'] == 'flight':
            return task['taskId'] in self.dh.crew_leg_map.get(crew_info['crewId'], set())
        elif task['type'] in ['positioning_flight', 'positioning_bus']:
            return True
        elif task['type'] == 'ground_duty':
            return task.get('crewId') == crew_info['crewId']
        return False
    
    def _evaluate_time_window_match(self, arrival_time, ground_duty):
        """
        评估到达时间与占位任务时间窗口的匹配度
        返回: (匹配分数, 等待时间)
        """
        gd_start = ground_duty['startTime']
        gd_end = ground_duty['endTime']
        
        # 计算等待时间
        wait_time = (gd_start - arrival_time).total_seconds() / 3600
        
        # 如果到达太晚，无法执行
        if arrival_time > gd_start:
            return 0.0, -1
        
        # 理想情况：提前30分钟到1小时到达
        if 0.5 <= wait_time <= 1.0:
            return 1.0, wait_time
        
        # 可接受：提前1-3小时
        elif 1.0 < wait_time <= 3.0:
            return 0.8, wait_time
        
        # 较长等待：3-6小时
        elif 3.0 < wait_time <= 6.0:
            return 0.5, wait_time
        
        # 过长等待：6-12小时
        elif 6.0 < wait_time <= 12.0:
            return 0.3, wait_time
        
        # 极长等待：超过12小时
        else:
            return 0.1, wait_time
    
    def _analyze_ground_duty_connections(self, action, crew_info, crew_state):
        """
        深度分析动作与占位任务的连接质量
        """
        arrival_airport = action.get('arriAirport')
        arrival_time = action['endTime']
        crew_id = crew_info['crewId']
        
        # 找出该机场的占位任务
        reachable_gds = []
        
        for gd in self.dh.crew_ground_duties.get(crew_id, []):
            if (gd['airport'] == arrival_airport and 
                gd['id'] in self.unassigned_ground_duty_ids and
                arrival_time <= gd['startTime']):
                
                # 评估时间匹配度
                match_score, wait_time = self._evaluate_time_window_match(arrival_time, gd)
                
                if match_score > 0:
                    reachable_gds.append({
                        'ground_duty': gd,
                        'match_score': match_score,
                        'wait_time': wait_time,
                        'urgency': self._calculate_ground_duty_urgency(gd)
                    })
        
        # 计算综合价值
        if reachable_gds:
            # 按综合价值排序
            reachable_gds.sort(key=lambda x: x['match_score'] * x['urgency'], reverse=True)
            
            # 返回最佳匹配的占位任务信息
            best_match = reachable_gds[0]
            
            # 计算动态奖励
            connection_value = 0
            for gd_info in reachable_gds:
                # 基础连接价值
                base_value = 100.0
                
                # 时间匹配奖励
                time_bonus = base_value * gd_info['match_score']
                
                # 紧急度加成
                urgency_bonus = base_value * gd_info['urgency'] * 0.5
                
                connection_value += time_bonus + urgency_bonus
            
            action['gd_connection_value'] = connection_value
            action['best_gd_match'] = best_match
            action['reachable_gd_details'] = reachable_gds
        
        return reachable_gds
    
    def _calculate_ground_duty_urgency(self, ground_duty):
        """计算占位任务的紧急度"""
        current_time = self.planning_start_dt + pd.Timedelta(hours=self.total_steps * 0.1)
        time_to_deadline = (ground_duty['endTime'] - current_time).total_seconds() / 3600
    ...
    def _get_valid_actions(self, crew_info, crew_state):
        ...
        if unfinished_gds:
            ...
            for action in all_candidate_tasks:
                ...
                if arrival_airport:
                    # 检查能否帮助到达占位任务
                    reachable_gds = [
                        gd for gd in unfinished_gds
                        if gd['airport'] == arrival_airport
                        and action['endTime'] <= gd['startTime']
                    ]
                    
                    if reachable_gds:
                        # 标记这个动作能帮助到达占位任务
                        action['helps_reach_gd'] = True
                        action['reachable_gd_count'] = len(reachable_gds)
                        
                        # 计算时间匹配奖励
                        time_bonus = 0
                        for gd in reachable_gds:
                            wait_time = (gd['startTime'] - action['endTime']).total_seconds() / 3600
                            if wait_time <= 6:  # 6小时内可以执行
                                time_bonus += 100.0
                            elif wait_time <= 12:  # 12小时内
                                time_bonus += 50.0
                        
                        action['gd_connection_bonus'] = time_bonus
        ...
        for action in all_candidate_tasks:
            ...
            if is_valid: 
                # 计算动态优先级分数（使用全局协调版本）
                action['dynamic_priority'] = self._calculate_dynamic_priority_with_coordination(action, crew_state, crew_info)
    ...
    def _calculate_dynamic_priority(self, task, crew_state, crew_info):
        ...
        if task['type'] == 'flight':
            ...
            if task.get('crewId') == crew_info['crewId']:
                ...
                # 非专属占位任务不允许执行（严格约束）
                base_ground_score -= 1000.0  # 完全排除非专属占位任务
        ...
        # 6. 新增：前瞻奖励 - 如果这个动作能帮助到达占位任务
        if task.get('helps_reach_gd'):
            gd_count = task.get('reachable_gd_count', 0)
            time_bonus = task.get('gd_connection_bonus', 0)
            
            # 基础连接奖励
            priority_score += 150.0 * gd_count
            
            # 时间匹配奖励
            priority_score += time_bonus * 0.5  # 降低权重避免过度奖励
            
            # 对置位任务给予额外奖励，因为它们能帮助到达占位任务
            if 'positioning' in task.get('type', ''):
                priority_score += 100.0  # 置位任务连接占位任务的额外奖励
        
        return priority_score

    def _calculate_dynamic_priority_with_coordination(self, task, crew_state, crew_info):
        """集成全局协调的优先级计算"""
        # 获取原有优先级
        base_priority = self._calculate_dynamic_priority(task, crew_state, crew_info)
        ...
        if self.global_coordinator:
            ...
            if arrival_airport:
                # 机场优先级奖励
                airport_bonus = self.global_coordinator.get_airport_priority_bonus(arrival_airport)
                base_priority += airport_bonus
                
                # 如果全局协调器建议去这个机场，额外奖励
                suggestion = self.global_coordinator.suggest_crew_movement(crew_info, crew_state)
                if suggestion and suggestion['target_airport'] == arrival_airport:
                    base_priority += 30.0 * suggestion['urgency']
    ...
    def step(self, action_idx):
        ...
        if action_idx < 0 or action_idx >= len(actions_to_consider):
            ...
            if task_type == 'flight':
                ...
                if task['taskId'] in self.unassigned_ground_duty_ids: 
                    ...
                    # 里程碑奖励：占位任务完成奖励
                    crew_total_gds = len(self.dh.crew_ground_duties.get(crew_id, []))
                    crew_completed_gds = crew_total_gds - len([gd for gd in self.dh.crew_ground_duties.get(crew_id, []) if gd['id'] in self.unassigned_ground_duty_ids])
                    
                    if crew_total_gds > 0:
                        completion_rate = crew_completed_gds / crew_total_gds
                        # 里程碑奖励：25%, 50%, 75%, 100%完成时给予递增奖励
            ...
            # 前瞻奖励：如果选择的动作能帮助到达占位任务
            if hasattr(task, 'helps_reach_gd') and task.helps_reach_gd:
                reward += task.get('gd_connection_bonus', 0) * 2.0  # 放大前瞻奖励
                reward += task.get('reachable_gd_count', 0) * 3.0   # 每个可达占位任务额外奖励
        ...
        # 定期更新全局协调状态
        if self.total_steps % (len(self.crews) * 2) == 0 and self.global_coordinator:
            self.global_coordinator.update_global_state()
    ...
    def _calculate_positioning_strategic_value(self, task, crew_info):
        ...
        crew_ground_duties = self.dh.crew_ground_duties.get(crew_id, [])
        direct_gds = [
            gd for gd in crew_ground_duties
            if gd['airport'] == arrival_airport
            and gd['id'] in self.unassigned_ground_duty_ids
            and task['endTime'] <= gd['startTime']
        ]
        
        if direct_gds:
            strategic_value += 200.0 * len(direct_gds)  # 直接可达占位任务的高价值
            # 直达占位任务价值计算
        ...
        for _, next_task in future_tasks.iterrows():
            next_airport = next_task['arriAirport']
            # 检查next_airport是否有占位任务
            indirect_gds = [
                gd for gd in crew_ground_duties
                if gd['airport'] == next_airport
                and gd['id'] in self.unassigned_ground_duty_ids
                and next_task['endTime'] <= gd['startTime']
            ]
            if indirect_gds:
                second_hop_value += 50.0 * len(indirect_gds)
        
        if second_hop_value > 0:
            strategic_value += second_hop_value
            # 二步到达占位任务价值计算
