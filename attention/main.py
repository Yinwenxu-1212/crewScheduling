...
def train():
    ...
    # 统计占位任务信息
    total_ground_duties = len(data_handler.data['ground_duties'])
    crew_ground_duties_count = {crew_id: len(duties) for crew_id, duties in data_handler.crew_ground_duties.items()}
    total_crews_with_ground_duties = len(crew_ground_duties_count)
    print(f"\n=== 占位任务统计 ===")
    print(f"总占位任务数量: {total_ground_duties}")
    print(f"有占位任务的机组数量: {total_crews_with_ground_duties}")
    print(f"平均每机组占位任务数: {total_ground_duties/max(1, total_crews_with_ground_duties):.1f}")
    print(f"占位任务分布: 最少{min(crew_ground_duties_count.values()) if crew_ground_duties_count else 0}个, 最多{max(crew_ground_duties_count.values()) if crew_ground_duties_count else 0}个")
    ...
    # 多GPU支持：如果有多个GPU可用，使用DataParallel
    if torch.cuda.device_count() > 1:
        print(f"使用 {torch.cuda.device_count()} 个GPU进行训练")
        model = torch.nn.DataParallel(model)
    else:
        print(f"使用单GPU/CPU进行训练: {config.DEVICE}")
    ...
    for episode in episode_pbar:
        ...
        while not (terminated or truncated):
            ...
            if not info['valid_actions']:
                ...
                with torch.no_grad():
                    action_probs, value = model(state_tensor, candidate_action_tensor, action_mask_tensor)
                    ...
                    try:
                        ...
                        for i in range(batch_size):
                            valid_indices = torch.where(action_mask_tensor[i])[0]
                    ...
                    # 计算各部分贡献
                    base_encoded = original_model.base_feature_extractor(base_features)
                    dual_encoded = original_model.dual_price_extractor(dual_features)
                    combined_features = torch.cat([base_encoded, dual_encoded], dim=-1)
                    actions_encoded = original_model.feature_fusion(combined_features)
                    
                    # 计算双头评分
                    action_scores = original_model.action_scorer(actions_encoded).squeeze(-1)
                    dual_values = original_model.dual_value_head(actions_encoded).squeeze(-1)
                    
                    # 获取自适应权重
                    alpha, beta = original_model._compute_adaptive_weights(action_scores, dual_values, state_tensor)
        ...
        # 统计占位任务分配情况
        assigned_ground_duties = 0
        total_assigned_tasks = 0
        for crew_id, roster in env.roster_plan.items():
            crew_ground_duties = sum(1 for task in roster if task.get('type') == 'ground_duty')
            assigned_ground_duties += crew_ground_duties
            total_assigned_tasks += len(roster)
        
        print(f"Episode {episode+1} 占位任务分配: {assigned_ground_duties}/{total_ground_duties} ({assigned_ground_duties/total_ground_duties*100:.1f}%)")
        print(f"总任务分配: {total_assigned_tasks}, 其中占位任务: {assigned_ground_duties}")
        ...
        if episode_reward > best_score:
            ...
            torch.save(original_model.state_dict(), os.path.join(config.MODEL_SAVE_PATH, "best_model.pth"))
        ...
        if episode % 500 == 0 and episode > 0:
            ...
            if torch.cuda.is_available():
                ...
                tqdm.write(f"GPU内存: {gpu_allocated:.2f}GB / {gpu_memory:.2f}GB (缓存: {gpu_cached:.2f}GB)")
...
def train_model_with_replay(model, optimizer, experience_replay, episode):
    ...
    for ppo_epoch in range(config.PPO_EPOCHS):
        action_probs, value = model(old_states, old_candidate_actions, old_action_masks)
        
        # 创建Categorical分布
        try:
            dist = Categorical(probs=action_probs)
        except ValueError as e:
            print(f"Warning: Failed to create distribution in training: {e}")
            # 使用均匀分布作为fallback
    ...
    with torch.no_grad():
        while not done:
            if not info['valid_actions']:
                ...
                action_probs, _ = model(state_tensor, candidate_action_tensor, action_mask_tensor)
